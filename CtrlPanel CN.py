...
class MotorControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RoboMaster飞镖电机控制台")
        # 增加宽度以容纳左侧边栏
    ...
    def create_monitor_panel(self, parent):
        ...
        # 标题行
        headers = ["ID", "状态", "模式", "角度", "RPM", "扭矩", "温度", "总角度"]
        for col, text in enumerate(headers):
            lbl = ttk.Label(scrollable_frame, text=text, font=('', 9, ''))
        ...
        for i in range(MOTOR_NUM):
            # ID / Name
            ttk.Label(scrollable_frame, text=f"{i} ({motor_names[i]})", font=('', 9)).grid(row=i+1, column=0, padx=2, pady=2, sticky="w")
            
            # Status (Enabled/Stalled)
            lbl_stat = ttk.Label(scrollable_frame, text="DIS", font=('', 9), foreground="gray", width=5)
            lbl_stat.grid(row=i+1, column=1, padx=2, pady=2)
            
            # Mode
            lbl_mode = ttk.Label(scrollable_frame, text="0", font=('', 9), width=3)
            lbl_mode.grid(row=i+1, column=2, padx=2, pady=2)
            
            # Angle
            lbl_angle = ttk.Label(scrollable_frame, text="0", font=('', 9), width=6)
            lbl_angle.grid(row=i+1, column=3, padx=2, pady=2)
            
            # RPM
            lbl_rpm = ttk.Label(scrollable_frame, text="0", font=('', 9), width=5)
            lbl_rpm.grid(row=i+1, column=4, padx=2, pady=2)
            
            # Torque
            lbl_trq = ttk.Label(scrollable_frame, text="0", font=('', 9), width=5)
            lbl_trq.grid(row=i+1, column=5, padx=2, pady=2)
            
            # Temp
            lbl_tmp = ttk.Label(scrollable_frame, text="0°C", font=('', 9), width=4)
            lbl_tmp.grid(row=i+1, column=6, padx=2, pady=2)

            # Total Angle
            lbl_total_angle = ttk.Label(scrollable_frame, text="0", font=('', 9), width=8)
    ...
    def create_individual_controls(self, parent):
        ...
        ttk.Label(frame_indiv, text="目标速度").grid(row=0, column=1, padx=2)
        ttk.Label(frame_indiv, text="控制台").grid(row=0, column=2, padx=2, columnspan=3)
    ...
    def create_control_panel(self, parent):
        # 1. 串口设置区域
        frame_conn = ttk.LabelFrame(parent, text="CDC连接")
        frame_conn.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(frame_conn, text="端口号:").pack(side="left", padx=5)
        ...
        frame_sys = ttk.LabelFrame(parent, text="任务控制 (0x01)")
        frame_sys.pack(fill="x", padx=10, pady=5)
        
        btn_halt = ttk.Button(frame_sys, text="紧急停止 (0x00)", command=self.action_emergency_stop)
        btn_halt.pack(side="left", padx=5, fill="x", expand=True)
        # Style the stop button red if possible, or just text
        
        ttk.Label(frame_sys, text="设置任务:").pack(side="left", padx=5)
        self.run_task_var = tk.IntVar(value=1)
        ttk.Entry(frame_sys, textvariable=self.run_task_var, width=5).pack(side="left", padx=2)
        ttk.Button(frame_sys, text="设置运行任务", command=self.action_set_task).pack(side="left", padx=5)

        # 2. 快捷控制区域 (New Features)
        frame_actions = ttk.LabelFrame(parent, text="快捷操作")
        ...
        ttk.Label(frame_launch_1, text="摩擦轮控制:").pack(side="left")
        
        # Friction Speed Input
        ttk.Label(frame_launch_1, text="一级速度:").pack(side="left", padx=2)
        ttk.Entry(frame_launch_1, textvariable=self.friction_speed_var, width=6).pack(side="left", padx=2)

        ttk.Label(frame_launch_1, text="二级速度:").pack(side="left", padx=2)
        ttk.Entry(frame_launch_1, textvariable=self.friction_speed_2_var, width=6).pack(side="left", padx=2)
        
        ttk.Button(frame_launch_1, text="启动摩擦轮", command=self.action_prepare_launch).pack(side="left", padx=5)
        ttk.Button(frame_launch_1, text="停止摩擦轮", command=self.action_stop_friction).pack(side="left", padx=5)

        # Row 2: Feed & Launch
        frame_launch_2 = ttk.Frame(frame_actions)
        frame_launch_2.pack(fill="x", padx=5, pady=2)
        ttk.Label(frame_launch_2, text="发射操作:").pack(side="left")
        ttk.Button(frame_launch_2, text="进弹", command=self.action_feed_in).pack(side="left", padx=5)
        ttk.Button(frame_launch_2, text="退弹", command=self.action_feed_out).pack(side="left", padx=5)
        
        ttk.Button(frame_launch_2, text="一键启动(执行设置任务编号)", command=self.action_one_click_launch).pack(side="left", padx=5)

        # Loading Mechanism (Lift - ID 6)
        frame_load = ttk.Frame(frame_actions)
        frame_load.pack(fill="x", padx=5, pady=5)
        ttk.Label(frame_load, text="上弹控制 (Lift ID 6):").pack(side="left")
        ...
        ttk.Button(frame_load, text="复位 (Stall -6000)", command=self.action_reset_load).pack(side="left", padx=5)
        
        # Yaw Control (ID 4)
        frame_yaw = ttk.Frame(frame_actions)
        frame_yaw.pack(fill="x", padx=5, pady=5)
        ttk.Label(frame_yaw, text="Yaw控制 (ID 4):").pack(side="left")
        ...
        ttk.Button(frame_yaw, text="设置", command=self.send_yaw_command).pack(side="left", padx=5)
        ...
        ttk.Label(frame_mode, text="模式:").pack(side="left")
        ...
        ttk.Label(frame_val, text="数值 (int16):").pack(side="left")
        ...
        ttk.Label(frame_val, text="时间 (ms):").pack(side="left", padx=5)
        ...
        ttk.Button(frame_btns, text="发送指令", command=self.send_packet).pack(side="left", fill="x", expand=True, padx=5)
        ttk.Button(frame_btns, text="停止所有 (Send Disable)", command=self.stop_motor).pack(side="left", fill="x", expand=True, padx=5)
        
        # 4. 日志区域
        frame_log = ttk.LabelFrame(parent, text="日志 (TX/Raw RX)")
